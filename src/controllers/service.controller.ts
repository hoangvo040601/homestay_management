...
const addServiceToBooking = async(req:Request,res:Response)=>{
    try {
        const bookingId = getParamId(req);
        const serviceBooking = await serviceServices.addServiceToBooking(bookingId, req.body)
        sendSuccess(res, serviceBooking, 'Thêm dịch vụ cho booking thành công!',201)
    } catch (error:any) {
        sendError(res,error.message, 400)
    }
}
...
const deleteServiceFromBooking = async(req:Request, res:Response)=>{
    try {
        ...
        const serviceBooking = await serviceServices.removeServiceFromBooking(bookingId,bookingServiceId)
        sendSuccess(res, serviceBooking)
    } catch (error:any) {
        sendError(res, error.message, 400)
    }
...
export default {getServices,getServiceById,createService,updateService,deleteService,
    getServiceBookings,addServiceToBooking,deleteServiceFromBooking
}